...
class Head(nn.Module):
    """
    A single attention head for self-attention mechanism.
    Projects input embeddings into key, query, and value spaces,
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self, vocab_size):
        ...
        self.positional_embedding_table = nn.Embedding(block_size, n_embd) # block_size, n_embd
    ...
    def forward(self, idx, targets=None):
        ...
        pos_emb = self.positional_embedding_table(torch.arange(idx.size(1), device=idx.device)) # T, C
        x = token_emb + pos_emb
